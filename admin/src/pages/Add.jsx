...
const PropertyForm = () => {
  ...
  const handleImageChange = (e) => {
    ...
    const total = files.length + previewUrls.length;
  
    if (total > 4) {
      alert(`You can only upload 4 images total. You selected ${files.length}, but already have ${previewUrls.length}.`);
      return;
    }
  ...
  //   formData.append("image", selectedFile); // selectedFile = file from input
  
  //   try {
  //     const res = await axios.post("http://localhost:4000/api/properties", formData, {
  //       headers: {
  //         "Content-Type": "multipart/form-data",
  //       },
  //     });
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.images[0]) {
      formDataToSend.append("image", formData.images[0]); // assuming one image for now
    }
  
    try {
      const res = await axios.post("http://localhost:4000/api/properties", formDataToSend, {
